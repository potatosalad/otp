...
write(Term, Options) when is_list(Options) ->
    ...
    if
        ...
        is_integer(CharsLimit), CharsLimit < 0, is_integer(Depth) ->
            write1(Term, Depth, Encoding, OrderedMaps);
        is_integer(CharsLimit), CharsLimit > 0 ->
            RecDefFun = fun(_, _) -> no end,
            If = io_lib_pretty:intermediate
                 (Term, Depth, CharsLimit, RecDefFun, Encoding, _Str=false, OrderedMaps),
...
write_map(_, 1, _E, _O) -> "#{}";
write_map(Map, D, E, O = true) when is_integer(D) ->
    case lists:sort(maps:keys(Map)) of
        [K | Ks] ->
            V = maps:get(K, Map),
            D0 = D - 1,
            W = write_map_assoc(K, V, D0, E, O),
            [$#, ${, [W | write_map_body_ordered(Ks, Map, D0, D0, E, O)], $}];
        [] ->
            "#{}"
    end;
...
write_map_body_ordered(_Ks, _Map, 1, _D0, _E, _O) ->
    ",...";
write_map_body_ordered([], _Map, _D, _D0, _E, _O) ->
    "";
write_map_body_ordered([K | Ks], Map, D, D0, E, O) ->
    V = maps:get(K, Map),
    W = write_map_assoc(K, V, D0, E, O),
    [$,, W | write_map_body_ordered(Ks, Map, D - 1, D0, E, O)].
