...
print(Term, Options) when is_list(Options) ->
    ...
    OrderedMaps = get_option(ordered_maps, Options, false),
    print(Term, Col, Ll, D, M, T, RecDefFun, Encoding, Strings, OrderedMaps);
...
print(Term, Col, Ll, D) ->
    print(Term, Col, Ll, D, _M=-1, _T=-1, no_fun, latin1, true, false).
...
print(Term, Col, Ll, D, M, RecDefFun) ->
    print(Term, Col, Ll, D, M, _T=-1, RecDefFun, latin1, true, false).
...
print_length_map(Map, 1, _T, RF, Enc, Str, Ord) ->
    ...
    {"#{...}", 6, 3, More};
print_length_map(Map, D, T, RF, Enc, Str, Ord = true) when is_map(Map) ->
    Keys = lists:sort(maps:keys(Map)),
    PairsS = print_length_map_pairs_ordered(Keys, Map, D, D, tsub(T, 3), RF, Enc, Str, Ord),
...
print_length_map_pairs_ordered([], _M, _D, _D0, _T, _RF, _Enc, _Str, _Ord) ->
    [];
print_length_map_pairs_ordered(Ks, M, D, D0, T, RF, Enc, Str, Ord) when D =:= 1; T =:= 0->
    More = fun(T1, Dd) ->
                   ?FUNCTION_NAME(Ks, M, D+Dd, D0, T1, RF, Enc, Str, Ord)
           end,
    {dots, 3, 3, More};
print_length_map_pairs_ordered([K | Ks], M, D, D0, T, RF, Enc, Str, Ord) ->
    V = maps:get(K, M),
    T1 =
        case Ks of
            [_ | _] -> tsub(T, 1);
    ...
    {_, Len1, _, _} = Pair1,
    [Pair1 |
     print_length_map_pairs_ordered(Ks, M, D - 1, D0, tsub(T1, Len1), RF, Enc, Str, Ord)].
