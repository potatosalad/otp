...
print([#{control_char := C, width := F, adjust := Ad, precision := P,
         ...
         ordered_maps := OrderedMaps} | Cs]) ->
    print(C, F, Ad, P, Pad, Encoding, Strings, OrderedMaps) ++ print(Cs);
print([C | Cs]) ->
    [C | print(Cs)];
print([]) ->
    [].

print(C, F, Ad, P, Pad, Encoding, Strings, OrderedMaps) ->
    [$~] ++ print_field_width(F, Ad) ++ print_precision(P, Pad) ++
        print_pad_char(Pad) ++ print_encoding(Encoding) ++
        print_strings(Strings) ++ print_ordered_maps(OrderedMaps) ++
        [C].
...
print_ordered_maps(false) -> "";
print_ordered_maps(true) -> "k".
...
modifiers([$k|Fmt], Spec) ->
    modifiers(Fmt, Spec#{ordered_maps => true});
modifiers(Fmt, Spec) ->
    {Spec, Fmt}.
...
build_limited([#{control_char := C, args := As, width := F, adjust := Ad,
                 ...
                 strings := Str, ordered_maps := Ord} | Cs],
